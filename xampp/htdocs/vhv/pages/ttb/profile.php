  ...
  */
  ...
  function createQueryForDeleteProfile($key)
    ...
    $Timestamp  = date('Y-m-d G:i:s');
		$sql = "UPDATE `patient` 
            SET `Status` = 'invalid', `Modified` = '$Timestamp', `Accessed` = '$Timestamp'
            WHERE IdCard LIKE '%$key%'";
		return $sql;
  }
  ...
  function createQueryUpdateTimeAccessProFile($key)
	{
    $Timestamp  = date('Y-m-d G:i:s');
    $sql   = "UPDATE `patient` SET `Accessed`  = '$Timestamp' WHERE  `IdCard` = '%$key%'";
		return $sql;
	}
  ...
  function updateTimeAccess($IdCard)
  {
    $Timestamp  = date('Y-m-d G:i:s');
    
    $sql   = "UPDATE `patient` 
              SET  `Modified`  = '$Timestamp', `Accessed`  = '$Timestamp'
              WHERE  `IdCard` = '%$IdCard%'"; 
      ...
      <script>
        function initMap(){
          ...
          */
          ...
          var typeColor = '1';
          var getTypeColor = '<?php echo $row['Type']; ?>';
          if(getTypeColor == 'A'){
            typeColor = '2';
          }else if(getTypeColor == 'B'){
            typeColor = '7';
          }else if(getTypeColor == 'C'){
            typeColor = '5';
          }else{
            typeColor = '1';
          }
          ...
          var markers = [
            ...
            
            {
              ...
              content:'<h3><?php echo $row['Name'];?></h3><br>พิกัด (<?php echo $row['Latitude'];?>, <?php echo $row['Longitude'];?>)'
            },
            {
